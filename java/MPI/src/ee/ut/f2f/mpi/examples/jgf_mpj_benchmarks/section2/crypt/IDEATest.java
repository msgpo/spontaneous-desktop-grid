...
void Do() throws MPIException
{
  ...
  if(rank==0) {
    ...
    for(int k=1;k<nprocess;k++){
     ...
     MPI().COMM_WORLD().Send(plain1,(p_array_rows*k),m_length,MPI.BYTE,k,k);
    }
  } else {
    MPI().COMM_WORLD().Recv(p_plain1,0,p_array_rows,MPI.BYTE,0,rank);
  }
  ...
  if(rank==0) {
    ...
    MPI().COMM_WORLD().Send(p_plain2,0,p_array_rows,MPI.BYTE,0,rank);
  }
