 ...
 * <li>Some text output seem to struggle to get out, without the user tapping ENTER on the keyboard!</li>
 * </ol>
 * 
 * @author H W Yau
 */
public class AppDemo extends Universal {
	...
	private java.util.Vector<ToTask> tasks;
	private java.util.Vector<ToResult> results;

	/**
	 * temporary vector for MPI
	 */
	private java.util.Vector<ToResult>[] p_results = new java.util.Vector[1];

	public AppDemo(JGFSection3 nJGFMonteCarloBench, String dataDirname, String dataFilename, int nTimeStepsMC, int nRunsMC) {
		this.JGFMonteCarloBench = nJGFMonteCarloBench;
	...
	/**
	 * Single point of contact for running this increasingly bloated class. Other run modes can later be defined for whether a new rate should be loaded in, etc. Note that if the <code>hostname</code>
	 * is set to the string "none", then the demonstrator runs in purely serial mode.
	 */
	...
	public void initSerial() {
		try {
			...
			RatePath rateP = new RatePath(JGFMonteCarloBench, dataDirname, dataFilename);
			rateP.dbgDumpFields();
	...
	public void runSerial() throws MPIException {
		...
		if (JGFMonteCarloBench.rank == 0) {
			results = new java.util.Vector<ToResult>(nRunsMC);
		}
		p_nRunsMC = (nRunsMC + JGFMonteCarloBench.nprocess - 1) / JGFMonteCarloBench.nprocess;
		p_results[0] = new java.util.Vector<ToResult>(p_nRunsMC);
		...
		if (JGFMonteCarloBench.rank == 0) {
			for (int i = 0; i < p_results[0].size(); i++) {
				results.addElement(p_results[0].elementAt(i));
			}
			for (int j = 1; j < JGFMonteCarloBench.nprocess; j++) {
				p_results[0].removeAllElements();
				JGFMonteCarloBench.MPI().COMM_WORLD().Recv(p_results, 0, 1, MPI.OBJECT, j, j);
				for (int i = 0; i < p_results[0].size(); i++) {
					results.addElement(p_results[0].elementAt(i));
				}
			...
			JGFMonteCarloBench.MPI().COMM_WORLD().Send(p_results, 0, 1, MPI.OBJECT, 0, JGFMonteCarloBench.rank);
	...
	private void initTasks(int nRunsMC) {
		tasks = new java.util.Vector<ToTask>(nRunsMC);
	...
	/**
	 * Method for doing something with the Monte Carlo simulations. It's probably not mathematically correct, but shall take an average over all the simulated rate paths.
	 * 
	...
	private void processResults() throws DemoException {
		...
		/*
		 * try{ Thread.sleep(200); } catch( InterruptedException intEx) { errPrintln(intEx.toString()); }
		 * 
		 */
	 ...
	 * 
	 * @param dataDirname
	 *            the value to set for the instance variable <code>dataDirname</code>.
	 */
	 ...
	 * 
	 * @param dataFilename
	 *            the value to set for the instance variable <code>dataFilename</code>.
	 */
	 ...
	 * 
	 * @param nTimeStepsMC
	 *            the value to set for the instance variable <code>nTimeStepsMC</code>.
	 */
	 ...
	 */
	public java.util.Vector<ToTask> get_tasks() {
		return (this.tasks);
	}
	 ...
	 */
	public void set_tasks(java.util.Vector<ToTask> tasks) {
		this.tasks = tasks;
	}
	// ------------------------------------------------------------------------
}
