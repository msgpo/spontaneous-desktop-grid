...
public abstract class SOR extends JGFSection2 {

	public SOR() {
		super();
	}
	public SOR(MPITask task) {
		super(task);
	}

	public double Gtotal = 0.0;

	public final void SORrun(double omega, double p_G[][], int num_iterations, double G[][]) throws MPIException {
		...
		if (rank == 0) {
			...
			for (int k = 1; k < nprocess; k++) {
				if (rank == k) {
					...
					MPI().COMM_WORLD().Send(p_G, 1, JGFSORBench.p_row, MPI.OBJECT, 0, rank);
				}
			}
		}
