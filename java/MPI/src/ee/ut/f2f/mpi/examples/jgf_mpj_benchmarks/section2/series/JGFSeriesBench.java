...
public class JGFSeriesBench extends SeriesTest {
	...
	public JGFSeriesBench(MPITask task) {
		super(task);
	}
	...
	public void JGFinitialise() {
		...
		/*
		 * determine the array dimension size on each process p_array_rows will be smaller on process (nprocess-1). ref_p_array_rows is the size on all processes except process (nprocess-1),
	...
	public void JGFvalidate() {
		...
		/*
		 * // for 200 points double ref[][] = {{2.8377707562588803, 0.0}, {1.0457844730995536, -1.8791032618587762}, {0.27410022422635033, -1.158835123403027}, {0.08241482176581083,
		 * -0.8057591902785817}};
		 */
		...
		if (rank == 0) {
			for (int i = 0; i < 4; i++) {
				for (int j = 0; j < 2; j++) {
					...
					if (error > 1.0e-12) {
						getMPIDebug().println("Validation failed for coefficient " + j + "," + i);
						getMPIDebug().println("Computed value = " + TestArray[j][i]);
						getMPIDebug().println("Reference value = " + ref[i][j]);
					}
				}
			}
		}
