 ...
 * 
 * @author H W Yau
 * @version $Revision: 1.2 $ $Date: 2007-07-17 08:27:06 $
 */
public class RatePath extends PathId {
	...
	/**
	 * Class variable for determining which field in the stock data should be used. This is currently set to point to the 'closing price'.
	 */
	public static int DATUMFIELD = 4;
	/**
	 * Class variable to represent the minimal date, whence the stock prices appear. Used to trap any potential problems with the data.
	 */
	public static final int MINIMUMDATE = 19000101;
	/**
	 * Class variable for defining what is meant by a small number, small enough to cause an arithmetic overflow when dividing. According to the Java Nutshell book, the actual range is
	...
	JGFSection3 JGFMonteCarloBench = null;

	/**
	 * Constructor, where the user specifies the directory and filename in from which the data should be read.
	 * 
	 * @param String
	 *            dirName
	 * @param String
	 ...
	 */
	public RatePath(JGFSection3 nJGFMonteCarloBench, String dirName, String filename) throws DemoException {
		JGFMonteCarloBench = nJGFMonteCarloBench;
		set_prompt(prompt);
		set_DEBUG(DEBUG);
	...
	/**
	 * Constructor, for when the user specifies simply an array of values for the path. User must also include information for specifying the other characteristics of the path.
	 * 
	 * @param pathValue
	 ...
	 * @param startDate
	 *            date from which the path is supposed to start, in 'YYYYMMDD' format.
	 * @param startDate
	 *            date from which the path is supposed to end, in 'YYYYMMDD' format.
	 * @param dTime
	 *            the time interval between successive path values, in fractions of a year.
	 */
	...
	/**
	 * Constructor, for use by the Monte Carlo generator, when it wishes to represent its findings as a RatePath object.
	 * 
	 * @param mc
	 *            the Monte Carlo generator object, whose data are to be copied over.
	 * @exception DemoException
	...
	// ------------------------------------------------------------------------
	/**
	 * Routine to update this rate path with the values from another rate path, via its pathValue array.
	 * 
	 * @param operandPath
	 *            the path value array to use for the update.
	 * @exception DemoException
	 *                thrown if there is a mismatch between the lengths of the operand and target arrays.
	 */
	 ...
	 * @exception DemoException
	 *                thrown if instance variable <code>pathValue</code> is undefined.
	 */
	 ...
	 * @exception DemoException
	 *                thrown if instance variable <code>pathDate</code> is undefined.
	 */
	...
	// ------------------------------------------------------------------------
	/**
	 * Method to return the terminal value for a given rate path, as used in derivative calculations.
	 * 
	...
	/**
	 * Method to return the value for a given rate path, at a given index. <i>One may want to index this in a more user friendly manner!</i>
	 * 
	 * @param index
	...
	/**
	 * Method for calculating the returns on a given rate path, via the definition for the instantaneous compounded return. u_i = \ln{\frac{S_i}{S_{i-1}}}
	 * 
	...
	/**
	 * Method for calculating the returns on a given rate path, via the definition for the instantaneous non-compounded return. u_i = \frac{S_i - S_{i-1}}{S_i}
	 * 
	 * @return the return, as defined.
	 * @exception DemoException
	 *                thrown if there is a problem with the calculation.
	 ...
	 * <li>Open Interest</li>
	 * </ol>
	 * One will probably make use of the closing price, but this can be redefined via the class variable <code>DATUMFIELD</code>. Note that since the read in data are then used to compute the
	 * return, this would be a good place to trap for zero values in the data, which will cause all sorts of problems.
	 * 
	 * @param dirName
	...
	private void readRatesFile(String dirName, String filename) throws DemoException {
		...
		java.io.BufferedReader in = JGFMonteCarloBench.getFailFromClasspath(dirName + "/" + filename);
		/*
		 * if (!ratesFile.canRead()) { throw new DemoException("Cannot read the file " + ratesFile.toString()); } try { in = new BufferedReader(new FileReader(ratesFile)); } catch
		 * (FileNotFoundException fnfex) { throw new DemoException(fnfex.toString()); }//
		 */
		//
		...
		iLine = 0;
		for (java.util.Enumeration<String> enum_ = allLines.elements(); enum_.hasMoreElements();) {
			aLine = enum_.nextElement();
