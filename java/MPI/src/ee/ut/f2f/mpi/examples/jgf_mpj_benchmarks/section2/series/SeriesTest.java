...
* (0.0824148, -0.805759).
*/
package ee.ut.f2f.mpi.examples.jgf_mpj_benchmarks.section2.series;
...
abstract class SeriesTest extends JGFSection2 
{
...
public SeriesTest(MPITask task) {
	super(task);
}
...
void Do() throws MPIException
{
    ...
    int ilow;
//getMPIDebug().println(" Do "+JGFSeriesBench.rank);
...
//getMPIDebug().println(" Do(1) "+JGFSeriesBench.rank);
...
//getMPIDebug().println(" Do(2) "+JGFSeriesBench.rank);
//getMPIDebug().println(" p_array_rows "+p_array_rows);
//System.exit(0);
    for (int i = ilow; i < p_array_rows; i++)
    {
	    //getMPIDebug().println("i :"+i);
        // Calculate A[i] terms. Note, once again, that we
        // can ignore the 2/period term outside the integral
        // since the period is 2 and the term cancels itself
        // out.

//getMPIDebug().println(" Do(3) "+JGFSeriesBench.rank);
        p_TestArray[0][i] = TrapezoidIntegrate((double)0.0,
                          (double)2.0,
                          1000,
                          omega * ((double)i + (ref_p_array_rows*rank)),
                          1);                       // 1 = cosine term.
...
//getMPIDebug().println("barrier starts "+ JGFSeriesBench.rank);
    MPI().COMM_WORLD().Barrier();
//getMPIDebug().println("barrier ends "+JGFSeriesBench.rank);
...
//getMPIDebug().println("recv "+JGFSeriesBench.rank);
