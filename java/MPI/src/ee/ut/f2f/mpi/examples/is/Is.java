...
public class Is extends MPITask {
	...
	void init() {
		...
		getMPIDebug().println("Size of buffers = " + SIZE_OF_BUFFERS);
	...
	void c_print_results(String name, char tclass, int n1, int n2, int n3, int niter, int nprocs_compiled, int nprocs_total, double t, double mops, String optype, int passed_verification, String npbversion, String compiletime, String mpicc, String clink, String cmpi_lib, String cmpi_inc,
			String cflags, String clinkflags) {
		getMPIDebug().println("\n\n" + name + " Benchmark Completed");
		getMPIDebug().println(" Class = " + tclass);
		if (n2 == 0 && n3 == 0)
			getMPIDebug().println("Size = " + n1);
		else
			getMPIDebug().println("Size = ");
	}
	...
	/***************************************************************************************************************************************************************************************************
	 * **************** R A N D L C ****************** **************** ****************** **************** protable random number generator ******************
	 **************************************************************************************************************************************************************************************************/
	double randlc(Object X, double A) {
		...
		/*
		 * Break X into two parts such that X = 2^23 * X1 + X2, compute Z = A* X2 + A2 * X1 (mod 2^23) and then X = 2^23 * Z + A2 * X2 (mod 2^46)
		 */
		T1 = R23 * lX[0];
		j = (int) T1;
		X1 = j;
	...
	void full_verify() {
		...
		for (i = 0; i < total_local_keys; i++) {
			...
			key_array[--(key_buff1[key_buff2[i] - min_key_val]) - total_lesser_keys] = key_buff2[i];
		}

		if (my_rank > 0) {
			req = MPI().COMM_WORLD().Irecv(ka, 0, 1, MPI.INT, my_rank - 1, 1000);
		}

		if (my_rank < comm_size - 1) {
			MPI().COMM_WORLD().Send(key_array, total_local_keys - 1, 1, MPI.INT, my_rank + 1, 1000);
		}
		...
		if (j != 0) {
			// getMPIDebug().println("Processor " + my_rank + ": Full_verify: number of key out of sort:" + j);
		} else {
			passed_verification++;
		}
	...
	void rank(int iteration) {
		...
		MPI().COMM_WORLD().Allreduce(bucket_size, 0, bucket_size_totals, 0, NUM_BUCKETS + TEST_ARRAY_SIZE, MPI.INT, MPI.SUM);
		...
		MPI().COMM_WORLD().Alltoall(send_count, 0, 1, MPI.INT, recv_count, 0, 1, MPI.INT);
		...
		MPI().COMM_WORLD().Alltoallv(key_buff1, 0, send_count, send_displ, MPI.INT, key_buff2, 0, recv_count, recv_displ, MPI.INT);
		...
		j = 0;
		for (i = process_bucket_distrib_ptr1[my_rank]; i <= process_bucket_distrib_ptr2[my_rank]; i++) {
			...
			if (min_key_val <= k && k <= max_key_val)
				...
				case 'S':
					if (i <= 2) {
						...
						if (key_buff1[k - 1 - min_key_val] != test_rank_array[i] + iteration) {
							...
							getMPIDebug().println("Failed partial verification:" + "iteration " + iteration + ", processor " + my_rank + ", test key " + i);
						} else
							passed_verification++;
					} else {
						// CHANGE: if(key_buff_ptr[k - 1] != test_rank_array[i]-iteration) {
				...
				case 'W':
					if (i < 2) {
						...
						if (key_buff1[k - 1 - min_key_val] != test_rank_array[i] + (iteration - 2)) {
							getMPIDebug().println("Failed partial verification:" + "iteration " + iteration + ", processor " + my_rank + ", test key " + i);
						} else
							passed_verification++;
					} else {
	...
	public void runTask() {
		...
		if (comm_size != NUM_PROCS) {
			if (my_rank == 0)
				getMPIDebug().println("\nERROR: not enough processes (code is compiled for NUM_PROCS=" + NUM_PROCS + ")");
			MPI().Finalize();
			return;
		...
		if (my_rank == 0) {
			getMPIDebug().println("NAS Parallel Benchmarks 3.1 -- IS Benchmark");
			getMPIDebug().println("Size : " + TOTAL_KEYS + " (class " + CLASS + ")");
			getMPIDebug().println("Iteration: " + MAX_ITERATIONS);
			getMPIDebug().println("Number of processes : " + comm_size);
		}

		/* Generate random number sequence and subsequent keys on all procs */
		create_seq(find_my_seed(my_rank, comm_size, 4 * TOTAL_KEYS, 314159265.00, 1220703125.00), 1220703125.00);

		// getMPIDebug().println("Starting... rank 1");
		rank(1);
		// getMPIDebug().println("Finishing... rank 1");
		...
		MPI().COMM_WORLD().Reduce(timecounter, 0, maxtime, 0, 1, MPI.DOUBLE, MPI.MAX, 0);
		...
		if (my_rank == 0) {
			...
			getMPIDebug().println("\t\tmin\t\tavg\t\tmax");
		}
		for (i = 1; i <= 3; i++) {
			...
			if (my_rank == 0) {
				getMPIDebug().println("timer" + i + "\t\t" + (tmin[0] / (double) 1000) + "\t\t" + (tsum[0] / (double) (comm_size * 1000)) + "\t\t" + (tmax[0] / (double) 1000));
			}
		}
		...
		MPI().COMM_WORLD().Reduce(itemp, 0, pass_v_array, 0, 1, MPI.INT, MPI.SUM, 0);
		...
		if (my_rank == 0) {
			...
			getMPIDebug().println("\n");
			getMPIDebug().println("IS Benchmark Completed\n");
			getMPIDebug().println("Class	    = " + CLASS);
			getMPIDebug().println("Size	    = " + TOTAL_KEYS);
			getMPIDebug().println("Iteration	    = " + MAX_ITERATIONS);
			getMPIDebug().println("Time in seconds = " + (maxtime[0] / 1000));
			getMPIDebug().println("Total processes = " + comm_size);
			double mopTotal = ((double) (MAX_ITERATIONS * TOTAL_KEYS)) / (maxtime[0] / 1000) / 1000000.0;

			getMPIDebug().println("Mop/s total     = " + mopTotal);
			getMPIDebug().println("Mop/s/process   = " + (mopTotal / comm_size));
