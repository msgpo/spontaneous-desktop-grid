...
package ee.ut.f2f.mpi.examples.jgf_mpj_benchmarks.section2.lufact;
...
public class JGFLUFactBench extends Linpack {
	...
	public JGFLUFactBench(MPITask task) {
		super(task);
	}
	...
	public void JGFinitialise() throws MPIException {
		...
		/*
		 * determine the size of the sub arrays and copy the data in a cyclic distribution to the sub arrays.
		 */
	...
	public void JGFvalidate() {
		...
		if (rank == 0) {
			...
			if (residn > ref[size]) {
				...
				getMPIDebug().println("Computed Norm Res = " + residn);
				getMPIDebug().println("Reference Norm Res = " + ref[size]);
			}
		}
