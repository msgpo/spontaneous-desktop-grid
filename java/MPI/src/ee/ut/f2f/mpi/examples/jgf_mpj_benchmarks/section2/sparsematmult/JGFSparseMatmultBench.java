...
package ee.ut.f2f.mpi.examples.jgf_mpj_benchmarks.section2.sparsematmult;
...
public class JGFSparseMatmultBench extends SparseMatmult{ 
  ...
  int p_datasizes_nz,ref_p_datasizes_nz,rem_p_datasizes_nz;
	...
	public JGFSparseMatmultBench(MPITask task) {
		super(task);
	}
...
*/
  ...
  if(rank==0) {
    ...
    for(int k=1;k<nprocess;k++) {
      ...
      MPI().COMM_WORLD().Send(buf_row,(k*ref_p_datasizes_nz),p_datasizes_nz,MPI.INT,k,1);
      MPI().COMM_WORLD().Send(buf_col,(k*ref_p_datasizes_nz),p_datasizes_nz,MPI.INT,k,2);
      MPI().COMM_WORLD().Send(buf_val,(k*ref_p_datasizes_nz),p_datasizes_nz,MPI.DOUBLE,k,3);
    }
  ...
  public void JGFkernel() throws MPIException{
    ...
    test(y, val, row, col, x, SPARSE_NUM_ITER, buf_row, p_y);
  ...
  public void JGFvalidate(){
    ...
    if(rank==0) {
      ...
      if (dev > 1.0e-12 ){
    	  ...
    	  getMPIDebug().println("ytotal = " + ytotal + "  " + dev + "  " + size);
      }
    }
