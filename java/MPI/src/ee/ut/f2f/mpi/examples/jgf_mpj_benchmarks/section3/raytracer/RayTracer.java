...
public abstract class RayTracer extends JGFSection3{ 
...
public void render(Interval interval) throws MPIException
  ...
  // Rendering variables
    ...
    // Header for .ppm file 
    // getMPIDebug().println("P3"); 
    // getMPIDebug().println(width + " " + height);
    // getMPIDebug().println("255"); 
    ...
    for(y = interval.yfrom+rank; y < interval.yto; y+=nprocess) {
      ...
      // getMPIDebug().println("Doing line " + y);
	...
	// RGB values for .ppm file 
        // getMPIDebug().println(red + " " + green + " " + blue); 
	// Sets the pixels
...
/* send temporary copies of p_row back to row */ 
    ...
    if(rank==0) {
     for(int k=0;k<nprocess;k++){
      ...
      MPI().COMM_WORLD().Send(p_row,0,p_row.length,MPI.INT,0,rank);
    }
