...
public class UDPTester extends Thread implements Activity, F2FMessageListener {
	 ...
	 */
	public void send(UDPTestMessage udpTestMessage)
			throws CommunicationFailedException {
		// send message
	 ...
	 * Send message and wait for remote peer to confirm
	 * reception
	 * 
	 * @param udpTestMessage
	 * @throws CommunicationFailedException
	 */
	public synchronized void blockingSend(UDPTestMessage udpTestMessage)
								throws CommunicationFailedException,
									   NotConfirmedException{
		String errMsg = "Failed to send message ["
			+ udpTestMessage.toString() + "] reception not confirmed";
		Exception ex = null;
		// send message
		remotePeer.sendMessage(udpTestMessage);
		// wait for confirmation
		try {
			udpTestMessage = blockingReceive();
			if (udpTestMessage.type == UDPTestMessage.Type.CONFIRM){
				return;
			}
		} catch (ReceiveQueueException e) {
			ex = e;
		}
		if (ex != null) throw new NotConfirmedException(errMsg,ex);
		else throw new NotConfirmedException(errMsg);
	}
	 ...
	 * arrives
	 */
	private synchronized UDPTestMessage blockingReceive()
			throws ReceiveQueueException {
		UDPTestMessage udpTestMessage = blockingReceiveQueue
				.getUDPTestMessage();
	 ...
	 * Method blocks execution until incoming message
	 * 
	 * @param desiredType type of message to wait for
	 * @param send confirm reception
	 */
	private synchronized UDPTestMessage blockingReceive(UDPTestMessage.Type desiredType, 
														boolean confirm)
											throws ReceiveQueueException, 
												   CommunicationFailedException,
												   UndesiredTypeException {
		// receive message
		UDPTestMessage udpTestMessage = blockingReceiveQueue.getUDPTestMessage();
		// send confirmation
		if (confirm){
			send(new UDPTestMessage(UDPTestMessage.Type.CONFIRM));
		}
		// check type
		if (udpTestMessage.type == desiredType) {
			return udpTestMessage;
		}
		throw new UndesiredTypeException("Message of type [" + udpTestMessage.type.toString() 
					+ "] received, but [" + desiredType.toString() + "] was desired");
	}
	...
	public void run() {
		...
		if (!UDPCommInitiator.isInitialized())
			return;
		...
		try {
			...
			ActivityManager.getDefault().emitEvent(
					new ActivityEvent(this, ActivityEvent.Type.FAILED, e
							.getMessage()));
		}
	...
	private void testProcess() throws CommunicationFailedException {
		...
		UUID localPeerId = F2FComputing.getLocalPeer().getID(); 
		UUID remotePeerId = remotePeer.getID();
		 ...
		 * ActivityEvent(this,ActivityEvent.Type.STARTED));
		 * log.debug(getActivityName() + "started");
		 * 
		 * // make sure that other peer has started the test too
		 * 
		 * ActivityManager.getDefault().emitEvent(new
		 * ActivityEvent(this,ActivityEvent.Type.CHANGED, "waiting for init"));
		 * Thread thread = new Thread() { public void run() { for (int i = 0; i
		 * < DEFAULT_WAITING_TIMEOUT; i++) { try { remotePeer.sendMessage(new
		 * UDPTestMessage()); if (status != Status.INIT) return;
		 * Thread.sleep(1000); } catch (Exception e) {} } } }; thread.start();
		 * while (true) { try { thread.join(); break; } catch
		 * (InterruptedException e) {} }
		 * 
		 * if (status == Status.INIT) {
		 * log.error("timeout while waiting for init from remote UDP test thread"
		 * ); ActivityManager.getDefault().emitEvent(new
		 * ActivityEvent(this,ActivityEvent.Type.FAILED,
		 * "timeout while waiting for init from remote UDP test thread")); //
		 * stop INIT sender setStatus(null); return; }
		 * 
		 * // exchange the STUN info remotePeer.sendMessage( new
		 * UDPTestMessage(localStunInfo)); // wait at most 30 seconds for remote
		 * addresses ActivityManager.getDefault().emitEvent(new
		 * ActivityEvent(this,ActivityEvent.Type.CHANGED,
		 * "waiting for addresses")); for (int i = 0; i <
		 * DEFAULT_WAITING_TIMEOUT; i++) { if (status == Status.GOT_STUN_INFO)
		 * break; try { Thread.sleep(500); } catch (InterruptedException e) {} }
		 * if (status != Status.GOT_STUN_INFO) {
		 * log.error("timeout while waiting for remote STUN info");
		 * ActivityManager.getDefault().emitEvent(new
		 * ActivityEvent(this,ActivityEvent.Type.FAILED,
		 * "timeout while waiting for remote STUN info")); return; } if
		 * (remoteStunInfo == null) { log.error("remoteStunInfo == null");
		 * ActivityManager.getDefault().emitEvent(new
		 * ActivityEvent(this,ActivityEvent.Type.FAILED,
		 * "remoteStunInfo == null")); return; }
		 * ActivityManager.getDefault().emitEvent(new
		 * ActivityEvent(this,ActivityEvent.Type.CHANGED,
		 * "got remote STUN info")); log.debug("got remote STUN info: " +
		 * remoteStunInfo);
		 * 
		 * //now remote and local STUN info is known -> try to establish UDP
		 * connection! log.debug("Local :\n" + "\tisOpenAccess : " +
		 * localStunInfo.isOpenAccess() + "\n" + "\tisBlockedUDP : " +
		 * localStunInfo.isBlockedUDP() + "\n" + "\tisSymmetricUDPFirewall : " +
		 * localStunInfo.isSymmetricUDPFirewall() + "\n" + "\tisFullCone() : " +
		 * localStunInfo.isFullCone() + "\n" + "\tisRestrictedCone : " +
		 ...
		 * "\n"); //decide which test should be run //if one of the sides has
		 * blocked UDP if(remoteStunInfo.isBlockedUDP() ||
		 * localStunInfo.isBlockedUDP()){ //UDP communication is impossible ->
		 * UDP blocked on one of the sides
		 ...
		 * //UDP communication not needed -> At least one of the sides has open
		 * access //TCP communication should be possiblelog.warn(
		 ...
		 * ActivityEvent.Type.FAILED, "UDP test failed"));
	...
	private void initUDPTests() throws CommunicationFailedException {
		...
		ActivityManager.getDefault().emitEvent(
				new ActivityEvent(this, ActivityEvent.Type.CHANGED,
		...
		if (localIas == null) {
			...
			ActivityManager.getDefault().emitEvent(
					new ActivityEvent(this, ActivityEvent.Type.FAILED,
		...
		while (createdConnections < MAX_UDP_CONNECTIONS
			...
			log.info(getActivityName() + "Mapped address\t["
			...
			if (this.status != Status.CONNECTION_ESTABLISHED) {
				...
				ActivityManager.getDefault().emitEvent(
						new ActivityEvent(this, ActivityEvent.Type.CHANGED,
								"connection established"));
				new Thread(new UDPConnection(localSocket, remoteMappedAddress,
						remotePeer)).start();
				createdConnections++;
			}
		}// for -udpConnections
	...
	private int portMappingRuleDiscovery(DatagramSocket localSocket)
		...
		log.debug(getActivityName() + " " + "Port Mapping Rule Discovery");
		ActivityManager.getDefault().emitEvent(
				new ActivityEvent(this, ActivityEvent.Type.CHANGED,
	...
	private InetSocketAddress resolveMappedAddress(DatagramSocket soc,
		...
		try {
			...
			log.error(getActivityName() + " " + "JSTUN utility exception", e);
			bytes = new byte[0];
			sendMh = null;
		}
		...
		if (!receiveMh.equalTransactionID(sendMh)) {
			log.warn(getActivityName() + " " + "Discard wrong packet");
		...
		if (!(ma == null)) {
			...
			try {
				...
				log.error(getActivityName() + " " + "JSTUN utility exception",
						e);
			} catch (UnknownHostException e) {
				log.error(getActivityName() + " " + e.getMessage());
			}
	...
	private InetSocketAddress resolveMappedAddress(DatagramSocket soc)
		...
		log.debug(getActivityName() + " " + "Resolving Mapped Address");
		ActivityManager.getDefault().emitEvent(
				new ActivityEvent(this, ActivityEvent.Type.CHANGED,
