	...
	private class StunServersFilteringThread extends Thread implements Activity {
		...
		public void run() {
			ActivityManager.getDefault().emitEvent(
					new ActivityEvent(this, ActivityEvent.Type.STARTED));
			...
			Collection<String> rawStunServers = F2FProperties
					.getF2FProperties().getSTUNProperties().stunServers;
			...
			log.info("Finished filtering reschable StunServers\n"
					+ (sb.length() == 0 ? "\tNo reachable servers" : sb
							.toString()));
			ActivityManager.getDefault().emitEvent(
					new ActivityEvent(this, ActivityEvent.Type.FINISHED));
			synchronized (LocalStunInfo.class) {
