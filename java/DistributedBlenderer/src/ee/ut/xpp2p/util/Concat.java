...
public class Concat implements ControllerListener, DataSinkListener {
    ...
    /**
     * @author Vladimir Ðkarupelov
     * The method is similar to main method, it concatinates the video files in one. 
     * 
     * @param outputFileURL the url of the video file to be outputted
     * @param inputFileURL the urls of the video files to be concatinated
     * @return true if files are successfully concatinated, false otherwise.
     */
    public static boolean concatinateVideoFiles(String outputFileURL, String ... inputFileURL){
    	...
    	// Generate the input and output media locators.
    	MediaLocator iml[] = new MediaLocator[inputFileURL.length];
    	MediaLocator oml;

    	for (int i = 0; i < inputFileURL.length; i++) {
    	    if ((iml[i] = createMediaLocator((String)inputFileURL[i])) == null) {
    		System.err.println("Cannot build media locator from: " + inputFileURL[i]);
    		return false;
    	    }
    	}

    	if ((oml = createMediaLocator(outputFileURL)) == null) {
    	    System.err.println("Cannot build media locator from: " + outputFileURL);
    	    return false;
    	}
    	Concat concat  = new Concat();
